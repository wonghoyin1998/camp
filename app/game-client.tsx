      ...
      <section className="task-modal" role="dialog" aria-modal="true" aria-labelledby="task-title">
        ...
        {complete ? <div className="complete-banner">✓ GM 已確認完成，獎勵已存入隊伍庫存。</div> : submitted ? <div className="pending-banner">⌛ 已提交，等待 GM 在控場台確認。</div> : active ? (
          <div className="submit-area">
            ...
            <div className="task-outcome-actions"><button className="task-fail-button" disabled={busy} onClick={fail}>任務失敗</button><button className="challenge-button" disabled={busy || !toolReady || !miniComplete || !routeActive || !state.tasksOpen} onClick={() => void act("submitTask", { taskId: task.id, note: [toolEvidence, note.trim()].filter(Boolean).join("・"), ...toolPayload })}>{!routeActive ? "13:00 航線開拓後先可提交" : !state.tasksOpen ? "14:45 任務已全面停止" : !toolReady ? "睇完玩法後先確認出題" : !miniComplete ? "先完成上面的遊戲工具" : "提交完成，等 GM 確認"}</button></div>
          </div>
        ) : (
